...
export default function ProductsClient({ initialData }: { initialData: PaginatedProducts }) {
  ...
  const categories = useMemo(() => Array.from(new Set(data.items.map(i => i.category))).sort(), [data.items]);

  return (
    <div className="space-y-4">
      <Filters total={data.total} categories={categories} onQueryChange={fetchData} />
